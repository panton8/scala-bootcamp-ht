...
object ControlStructures1 {
  ...
  def isAdultIf(age: Int): Either[Error, Boolean] = {
    ...
    else if (age > 150) Left(s"$age is too high, are you human?")
    else if (age >= 18 && age <= 150) Right(true)
    else Right(false)
  }

  // same as isAdultIf, but use match statement instead
  def isAdultMatch(age: Int): Either[Error, Boolean] = age match {
    case value if value < 0 => Left(s"$age is negative, we do not serve unborn people")
    case value if value > 150 => Left(s"$age is too high, are you human?")
    case _ => Right(age >= 18)
  }
  ...
  def isValidCandidate(
    ...
    val pointForCountry = country match {
      case "Wakanda" => 3
      case "Narnia" | "Skyrim" | "Amestris" => 1
      case _ => 0
    }

    val pointsForTests = if (passedTests >= 5) passedTests - 5 else -1

    val pointForExperience = yearsOfExperience match {
      case value if (value > 1 && value <= 5) => yearsOfExperience
      case value if (value > 5) => 5
      case _ => 0
    }

    val pointsForEducation = if (hasEducation) 3 else 0

    val pointsForGitHubStars = gitHubStars(starsOnGithub, 0)

    if (pointsForTests == -1) false
    else (pointForCountry + pointsForTests + pointForExperience + pointsForEducation + pointsForGitHubStars) > 9
  }

  private def gitHubStars(stars: Int, points: Int): Int = {
    if (stars / 10 == 0) points else gitHubStars(stars / 10, points + 1)
  }
}
